...
class MedecineScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    OneMedicineCubit.get(context).getOneMedicine(medicines.id!);
    return Scaffold(
      appBar: AppBar(
        title: CustomText(
          text: "Mdecine",
        ...
        actions: [
          BlocBuilder<OneMedicineCubit, OneMedicineState>(
            builder: (context, state) {
              if (state is OneMedicineSuccess) {
                return IconButton(
                  onPressed: () {
                    context.pushNamed(
                      Routes.editPillScreen,
                      arguments: state.oneMedicine,
                    );
                  },
                  icon: const Icon(Icons.edit, color: ColorsManager.green),
                );
              }
      ...
      body: BlocBuilder<OneMedicineCubit, OneMedicineState>(
        builder: (context, state) {
          if (state is OneMedicineSuccess) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      CustomTextFeild(
                        title: "Pills name",
                        enabled: false,
                        hintText: state.oneMedicine.name,
                        ...
                        children: [
                          Expanded(
                            child: CustomTextFeild(
                              ...
                              enabled: false,
                              hintText: state.oneMedicine.type,
                              prefixIcon: const PillIcon(),
                            ),
                          ),
                          SizedBox(width: 20.w),
                          Expanded(
                            child: CustomTextFeild(
                              title: "count",
                              enabled: false,
                              hintText: state.oneMedicine.count.toString(),
                              prefixIcon: const PillIcon(),
                            ),
                          ),
                        ],
                        ...
                        children: [
                          Expanded(
                            child: CustomTextFeild(
                              title: "taken count",
                              enabled: false,
                              hintText: state.oneMedicine.takenCount.toString(),
                              prefixIcon: const PillIcon(),
                            ),
                          ),
                          SizedBox(width: 20.w),
                          Expanded(
                            child: CustomTextFeild(
                              title: "frequency",
                              enabled: false,
                              hintText: state.oneMedicine.frequency,
                              prefixIcon: const PillIcon(),
                            ),
                          ),
                        ],
                        ...
                        children: [
                          Expanded(
                            child: CustomTextFeild(
                              ...
                              enabled: false,
                              hintText: state.oneMedicine.beforeEating!
                                  ? "before"
                                  : "after",
                              prefixIcon: const PillIcon(),
                            ),
                          ...
                          Expanded(
                            child: CustomTextFeild(
                              ...
                              enabled: false,
                              hintText: state.oneMedicine.shouldReminder!
                                  ? "yes"
                                  : "no",
                              prefixIcon: const PillIcon(),
                            ),
                      ...
                      TitleWithValue(
                        title: "dosage",
                        value: state.oneMedicine.dosage!,
                      ),
                      SizedBox(height: 20.h),
                      TitleWithValue(
                        title: "Last Time Take",
                        value: state.oneMedicine.startTime!,
                      ),
                      SizedBox(height: 20.h),
                      // CustomText(
                      //   text: " History",
          ...
          } else if (state is OneMedicineError) {
            return Center(
              child: CustomText(
                text: state.errorMessage,
                style: TextStyleManager.textStyle15w500,
      ...
      bottomNavigationBar: BlocBuilder<OneMedicineCubit, OneMedicineState>(
        builder: (context, state) {
          if (state is OneMedicineSuccess) {
            return Padding(
                ...
                children: [
                  ...
                  Expanded(
                    child: CustomElevatedButton(
                      onPressed: () {
                        MedicinesCubit.get(context)
                            .takeMedicines(id: state.oneMedicine.id!);
                      },
                      child: CustomText(
                        text: "Take",
